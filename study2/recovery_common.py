...
def each_param_parallel(parameter_samples,valid_parameter_indices,weights):
	...
	# numpy rewrite of the original pandas implementation -- verified to return
	# identical mean, CI and (seeded) resamples, ~4x faster. The weighted mean,
	# weighted-CDF quantiles and cum_weights resampling are unchanged math.
	good_parameters=np.asarray(parameter_samples)[valid_parameter_indices]
	weights=np.divide(weights,np.sum(weights))
	order=np.argsort(good_parameters,kind='quicksort')
	sorted_params=good_parameters[order]
	cdf=np.cumsum(np.asarray(weights)[order])
	mean=np.sum(weights*good_parameters)
	samples=list(random.choices(sorted_params.tolist(),cum_weights=cdf.tolist(),k=1000))
	index_5=int(np.searchsorted(cdf,0.0499999,side='right'))
	index_95=int(np.searchsorted(cdf,0.9499999,side='right'))
	ci=[sorted_params[index_5],sorted_params[index_95]]
