...
subs_df=pd.read_csv('preprocessed_data.csv')
...
csv_final=[['sub','optimal_delayed_score','worry','stress','manipCheckWorry']]
...
for sub in subs:
	...
	for col in q_cols:
		...
		if counter in reverse_counters:
			...
			avg_rev+=reverse_dict[int(df_temp[0])]

		else:
			sum_pswq += int(df_temp[0])
			avg_reg+=int(df_temp[0])
	...
	avg_reg=avg_reg/(len(q_cols)-5)
	avg_rev=avg_rev/5
	check_PSWQ=np.abs(avg_reg-avg_rev)
	csv_final.append([sub,optimal_delayed_score_sum,sum_pswq,sum_stress,check_PSWQ])


with open('optimal_delayed_scores.csv'.format(num_decisions_to_consider), 'w', encoding='utf-8') as f:
	writer=csv.writer(f)
