...
critical_values = {'depth=1': 15, 'depth=2': 19.59, 'depth=3': 41.72}
...
# Create a FacetGrid
g = sns.FacetGrid(results_df, col="planning_depth", sharey=False)

# Plot the distributions
g.map(sns.histplot, 'score', kde=True)

# Add the critical value lines
for ax, depth in zip(g.axes.flat, ['depth1_score', 'depth2_score', 'depth3_score']):
    ax.axvline(x=critical_values[depth.replace('_score', '')], color='red', linewidth=2.5)
