...
sns.set(style='white', font_scale=2.5, palette='gist_heat')

df['optimally delayed control']=df['optimal_delayed_score']
df['memory performance']=df['percent_correct']

# Create the seaborn regression plot with jitter
sns.lmplot(x='memory performance', y='optimally delayed control',height=6,aspect=2,logistic=True, data=df, scatter=True)
